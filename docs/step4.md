...
### 2.1 ModelProperties 및 MultiLayerNetworkConfig 생성

모델 경로와 같은 설정을 코드에 하드코딩하지 않고 외부 설정 파일(`application.properties`)에서 관리하기 위해 `ModelProperties` 클래스를 사용합니다. 또한 모델 파일을 읽어 Spring Bean으로 등록하는 설정 클래스를 `config` 패키지에 생성합니다.

**[src/main/resources/application.properties]**
```properties
iris.model-path=model/iris-model.zip
```

**[src/main/java/com/nhnacademy/nhnacademyaiirisclassifier/config/ModelProperties.java]**
```java
...
**[src/main/java/com/nhnacademy/nhnacademyaiirisclassifier/config/MultiLayerNetworkConfig.java]**
```java
package com.nhnacademy.nhnacademyaiirisclassifier.config;
...
이제 `ModelProperties`를 주입받아 사용하도록 서비스를 수정합니다.
...
public class IrisModelService {
    ...
    // ... (기존 initModel 및 trainAndSaveModel 메서드에서 MODEL_PATH 대신 modelProperties.getModelPath() 사용)
...
1.  **`@ConfigurationProperties`**: `application.properties`에 정의된 `iris.model-path` 값을 자바 객체 필드에 자동으로 바인딩합니다. 이를 통해 설정 변경 시 코드를 수정할 필요가 없습니다.
2.  **순환 참조 해결**: `IrisModelService`는 `MultiLayerNetwork`가 필요하고, `MultiLayerNetwork` 설정 과정에서는 `IrisModelService`가 필요할 수 있습니다. 이를 해결하기 위해 서비스에서 `final`을 제거하고 `setModel` 메서드를 통해 나중에 모델을 전달받도록 설계했습니다.
3.  **`@Configuration & @Bean`**: 스프링이 켜질 때 `multiLayerNetwork()` 메서드를 실행하고, 반환된 모델 객체를 스프링 컨테이너가 관리합니다.
